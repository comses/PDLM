...
class Sensora(AtomicDEVS):
    ...
    def __init__(self, name=None, id=None, kwargs=None):
        super(Sensora, self).__init__(name)

        # State

        count = 0

        self._state = SensoraState(
            count,
        )
        ...
        print(f"[{self.current_time:9.3f}][INIT] Sensora-{self.name}: {self._state}")
    ...
    def intTransition(self):
        ...
        self.ta = ta
        print(f"[{self.current_time:9.3f}][INT] Sensora-{self.name}: {self._state}")
    ...
    def extTransition(self, inputs: dict):
        ...
        self.ta = ta
        print(f"[{self.current_time:9.3f}][EXT] Sensora-{self.name}: {self._state}")
